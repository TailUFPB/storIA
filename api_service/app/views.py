...
def init_routes(app):
    ...
    def submit():
        ...
        data = request.form
        input_text = data.getlist('text[]')[0]
        size = data.getlist('length[]')[0]
        temperature = data.getlist('temperature[]')[0]
        ...
        try:
            ...
            if (type(size) != type(1)) or (size < 20) or (size > 500):
                raise
            if (type(temperature) != type(1.2)) or (temperature < 0.0) or (temperature > 2.0):
                raise
