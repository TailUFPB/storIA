from flask import Response, g, Flask, request
...
def start_metrics(app: Flask) -> None:
    """Inicia a instrumentação de métricas no Flask."""
    @app.before_request
    def start_timer() -> None:
        g.start_time = time.time()

    @app.after_request
    def log_latency(response: Any) -> Any:
        if hasattr(g, 'start_time'):
            elapsed_time = (time.time() - g.start_time) * 1000  # milissegundos
            REQUEST_LATENCY.set(elapsed_time)
        return response
