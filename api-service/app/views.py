...
def generate_cache_key(text: str, size: int, temperature: float) -> str:
    """
    Gera uma chave única para o cache usando hash dos parâmetros.
    """
    hash_input = f"{text}:{size}:{temperature}".encode('utf-8')
    key_hash = hashlib.md5(hash_input).hexdigest()
...
def init_routes(app) -> None:
    ...
    def get_result(job_id):
        try:
            ...
            if job.is_finished:
                cache_key = generate_cache_key(job.args[0], job.args[1], job.args[2])
                redis_client.setex(cache_key, 3600, job.result)
                storia_logger.info(f"Resultado armazenado em cache para chave: {cache_key}")
